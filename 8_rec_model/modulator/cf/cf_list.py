...
def hhi(exp):
	...
	print(np.sum(exp[:,3]==0))
	print(np.sum(tot_exp==0))
	print(exp.shape)
	exp = exp[tot_exp!=0]
	tot_exp = tot_exp[tot_exp!=0]
	print(exp.shape)
	x = exp.T/tot_exp
...
def main():
	...
	data1 = pandas.read_csv('cf_data_jja.csv', low_memory=False).as_matrix()

	with open('item_asp_map_cf_true.pickle', 'rb') as handle:
		item_asp = pickle.load(handle)



	rev_dict = {}
	for item in item_asp:
		if item_asp[item] not in rev_dict:
			rev_dict[item_asp[item]] = [item]	
		else:
			rev_dict[item_asp[item]].append(item)
	sum = 0
	for asp in range(6):
		sum += len(rev_dict[asp])
	for asp in range(6):
		print(str(asp)+" "+str(len(rev_dict[asp])/sum))

	exit()

	with open('cf_items.pickle', 'rb') as handle: #item_id -> index
