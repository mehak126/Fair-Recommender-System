...
def main():
	...
	data = pd.read_csv('../Data/met2_45_45/features_equal_rates.csv', low_memory=False, header = None).as_matrix()
	...
	for key in aspect_item_dict_id.keys():
		...
		rating_item_dict[key] = []
		for item_id in aspect_item_dict_id[key]:
			item_mapping[item_counter] = item_id
			aspect_item_dict[key].append(item_counter)
			item_pos_data = mdd[item_id] #row no of item in data
			rating = data[item_pos_data][0]
			rating_item_dict[key].append(data[mdd[item_id]][0])
			item_counter += 1
	...
	for i in range(num_aspects):
		rem_time_div[i] = (rem_time*rem_time_ratio[i])/total_items
	...
	for i in range(num_aspects):
		...
		tot_asp_exp = rem_time_div[i] + min_time_per_aspect
		tot_rating_sum = np.sum(np.asarray(rating_item_dict[i]))
		count = 0
		for item in aspect_items:
			E_desired[item] = (rating_item_dict[i][count] / tot_rating_sum) * tot_asp_exp 
